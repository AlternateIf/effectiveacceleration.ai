...
const DashboardTabs = () => {
  ...
  const filteredJobsMemo = useMemo(() => {
    if (selectedJobs.length === 0) return { open: [], aplications: [], started: [], completed: [] };
    ...
    selectedJobs.forEach((job, index) => {
      ...
      } else if (job.state === JobState.Closed && localJob?.lastJobEvent?.type_ === JobEventType.Completed || localJob?.lastJobEvent?.type_ === JobEventType.Rated || localJob?.lastJobEvent?.type_ === JobEventType.Arbitrated) {
        // Completed Jobs
        filteredCompletedJobs.push(job);
      }
    });
    return { open: filteredOpenJobs, aplications: filteredJobsAplications, started: filteredStartedJobs, completed: filteredCompletedJobs  };
  }, [jobs, localJobs]);
  ...
  return (
    ...
    {mounted && (
      <Tabs key={tabsKey} selectedIndex={activeTabIndex} onSelect={index => setActiveTabIndex(index)}>
        ...
        <TabPanel>
          <AllJobs jobs={jobs.filter(job => job.state === 0)} localJobs={localJobs}/>
        </TabPanel>
        <TabPanel>
          <JobsAplications filteredJobs={filteredJobsAplications} localJobs={localJobs}></JobsAplications>
        </TabPanel>
        <TabPanel>
          <JobProgress filteredJobs={filteredStartedJobs} localJobs={localJobs}></JobProgress>
        </TabPanel>
        <TabPanel>
          <CompletedJobs filteredJobs={filteredCompletedJobs} localJobs={localJobs}></CompletedJobs>
        </TabPanel>
      </Tabs>
    )}
    </div>
