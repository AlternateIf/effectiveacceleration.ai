...
const DashboardTabs = () => {
  ...
  const filteredJobsMemo = useMemo(() => {
    ...
    selectedJobs.forEach((job, index) => {
      ...
      } else if (job.state === JobState.Closed && localJob?.lastJobEvent?.type_ === JobEventType.Closed) {
        filteredCancelledJobs.push(job);
      } else if (job.state === JobState.Taken && localJob?.lastJobEvent?.type_ === JobEventType.Disputed) {
